...
function sendit(tvIdx){
  ...
  let tvThumbnail;
  try{
    tvThumbnail=document.querySelector("#base").value;
    console.log('포스터'+tvThumbnail.value);
  }catch (exception){
    tvThumbnail=null;
  }
  ...
  let tvBackImg;
  try{
    tvBackImg=document.querySelector("#base3").value;
    console.log('배경사진'+tvBackImg.value);
  }catch (exception){
    tvBackImg=null;
  }
  ...
  let tvTitle = document.querySelector("#tvTitle");
  let tvTitleOrg = document.querySelector("#tvTitleOrg");
  let tvMakingDate =document.querySelector("#tvMakingDate");
  ...
  let tvCountry = myList1.substring(10,myList1.length-1)


  //장르
  let mygenre = document.querySelectorAll("#tvGenre + span li");
  let mygenre1;
  mygenre.forEach((element) => {
    mygenre1 += ','+element.title;
  });
  let tvGenre = mygenre1.substring(10,mygenre1.length-1)


  //채널
  let myChannel = document.querySelectorAll("#tvChannel + span li");
  let myChannel1;
  myChannel.forEach((element) => {
    myChannel1 += ','+element.title;
  });
  let tvChannel = myChannel1.substring(10,myChannel1.length-1)


  let tvAge= $("#tvAge option:selected");

  let people=document.querySelectorAll(".hiddenBox");
  let tvPeople;
  people.forEach(function(person) {
    tvPeople += ',' + person.childNodes[0].data;
  });
  try{
    tvPeople = tvPeople.substring(10,tvPeople.length)
  }catch{
    tvPeople = null;
  }

  let tvSummary = document.querySelector("#tvSummary");



  let tvGallery = null;
  if(gallerynum==1) {
    tvGallery = document.getElementById("base(0)").value;
  }
  else if(gallerynum>1){
    for(let i=0;i<gallerynum;i++){
      baseid = "base("+i+")";
      tvGallery += document.getElementById(baseid).value + "|";
    }
    tvGallery=tvGallery.substring(4,tvGallery.length-1);
  }
  ...
  if(idnum!=0){
    ...
    for(let i=0;i<=idnum;i++){
      ...
      tvVideo+=(document.getElementById(vu).value)+','+(document.getElementById(vt).value)+'|';
    }
    tvVideo=tvVideo.substring(4,tvVideo.length-1);
  }else {
    if (document.getElementById("vt0").value == "" && document.getElementById("vu0").value != "") {
      ...
      tvVideo = (document.getElementById("vu0").value + "," + document.getElementById("vt0").value);
    }
  }
...
let tvWatch=null;
  ...
  if(tempcntnum>0){
    tvWatch=tvWatch.substring(4,tvWatch.length-1);
  }




  console.log('제목'+tvTitle.value);
  console.log('원제목'+tvTitleOrg.value);
  console.log('개봉일'+tvMakingDate.value);
  console.log('채널'+tvChannel)
  console.log('장르'+tvGenre);
  console.log('국가'+tvCountry);
  console.log('나이'+tvAge.val());
  console.log('내용'+tvSummary.value);
  console.log('포스터'+tvThumbnail);
  console.log('배경사진'+tvBackImg);
  console.log('갤러리' + tvGallery);
  console.log('동영상제목' + tvVideo);
  console.log('감상가능한곳' + tvWatch);
  console.log('출연/제작' + tvPeople);



  if(tvTitle.value == ''){
    alert('표기할 프로그램명을 입력하세요');
    return false;
  }
  if(tvTitleOrg.value == ''){
    alert('원제를 입력하세요');
  ...
  fetch('http://localhost:8888/api/tv', {
    ...
    body: JSON.stringify({
      ...
      "data":{
        ...
        "tvGenre":`${tvGenre}`,
        "tvGallery":tvGallery,
        "tvMakingDate":`${tvMakingDate.value}`,
        "tvPeople":tvPeople,
        "tvSummary":tvSummary.value,
        "tvThumbnail":tvThumbnail,
        "tvTitle":`${tvTitle.value}`,
        "tvTitleOrg":`${tvTitleOrg.value}`,
        "tvVideo":tvVideo,
        "tvChannel":tvChannel,
        "tvWatch":tvWatch,
